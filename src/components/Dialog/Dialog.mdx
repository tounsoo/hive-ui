...
- **Auto Focus**: React's native `autoFocus` property often conflicts with the timing of `dialog.showModal()`, causing the focus to be lost or reset. Additionally, React 19 often removes the `autoFocus` attribute from the DOM entirely.
  > **Recommendation**: use the `data-autofocus` attribute on the element you wish to focus. The `Dialog` component specifically looks for this attribute after the modal opens to ensure reliable focus management.
