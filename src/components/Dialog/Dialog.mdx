...
Use `Dialog.Sticky` to create headers or footers that remain fixed while the content scrolls.

1.  **Wrapper Component**: Wrap your header/footer in `<Dialog.Sticky>`.
2.  **Alignment**: Set `align="top"` for headers or `align="bottom"` for footers.
3.  **Edge Flush**: The component automatically handles negative margins to sit flush against the dialog edges.

```tsx
<Dialog className="max-h-[500px]">
    {/* 1. Wrapper with Alignment */}
    <Dialog.Sticky align="top">
...
3.  **Scroll Prevention**: The component automatically prevents the browser from scrolling to the bottom (if the last element were focused by default), ensuring the user starts at the top of the content.
